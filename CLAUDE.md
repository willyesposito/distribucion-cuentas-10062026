...
- **v2 entregado (parcial)**: motor de ciclos puro `src/ciclos.js` (días hábiles + feriados AR, eventos fechados a partir de cut-off, carga diaria en horas, choques duro/advertencia, indicadores), heatmap analista × día `src/HeatmapCarga.jsx` con gradiente celeste→amarillo→rojo, indicadores antes/después (pico, choques duros, advertencias), toggle ×complejidad (factor C/3), popover de detalle por celda. Horas default por etapa (fuente PO Pilar 8179343211, `n_meros__1` = horas, confirmado por usuario): Mensual = Cut Off 1 · v1 6 · Comentarios 3 · v2 6 · Aprobación 2; Quincena = 1 · 3 · 2 · 3 · 1.
- **Mejoras UX entregadas (11/06/2026)**: (a) pesos del score editables también por **input numérico** (editan el valor crudo 0-100, al lado se ve el % normalizado); (b) **IDs de liquidación determinísticos** `clienteId:mensual / :q1 / :q2 / :extra-N` — los cut-offs (keyed por liq.id) sobreviven cambios de tipo de liquidación y son clave estable para import y escenarios (paso 4). Al eliminar/cambiar tipo de liq se **podan los cut-offs huérfanos** (`podarCutoffsHuerfanos`) para que un `:extra-N` reusado no resucite una fecha vieja; (c) panel `src/PanelCutoffs.jsx` **"Carga de cut-offs"** junto al calendario — reemplaza al viejo bloque "Pendientes de cut-off" del heatmap: tabla cliente × liquidación con date input inline, **defaults por mes objetivo (Mensual día 18 · Q1 día 16 · Q2 día 1 del mes siguiente**, regla del usuario, días editables; "Aplicar defaults" solo llena vacías de tipo Mensual/Q1/Q2, los extras a mano), **semáforo** verde=confirmado / amarillo=default sin confirmar / gris=vacía (✓ confirma sin cambiar fecha; editar una fecha siempre confirma), **"Pegar desde Excel"** (filas Cliente|Liquidación|Fecha, separador tab/;/, fechas DD/MM/YYYY o ISO; admite Cliente|Fecha si hay una sola liq; etiquetas duplicadas resueltas por orden; entra como confirmado) y **"Copiar plantilla"** (tab-separated DD/MM/YYYY, round-trip, fallback execCommand); lista aparte de clientes sin tipo de liquidación; (d) badge **"⚠ N cut-offs default sin confirmar"** en el heatmap (pico/choques sobre defaults son tentativos).
...
src/HeatmapCarga.jsx                  # (v2) calendario analista × día + indicadores antes/después + popover + badge "sin confirmar"
src/PanelCutoffs.jsx                  # (v2) carga rápida de cut-offs: defaults por mes objetivo, semáforo, pegado Excel, plantilla
